...
namespace Dummiesman {
	public class CharWordReader {
		...
		public Color32 ReadVertexColor()
		{
			this.SkipWhitespaces(out var newLinePassed);
			float r = 0f;
			if (newLinePassed == false)
            ...
            {
				throw new System.ArgumentException("no vertex color");
			}
			this.SkipWhitespaces(out var newLinePassed2);
			float g = 0f;
			if (newLinePassed2 == false)
			{
				g = this.ReadFloat();
			}
			this.SkipWhitespaces(out var newLinePassed3);
			float b = 0f;
			if (newLinePassed3 == false)
			{
				b = this.ReadFloat();
			}
